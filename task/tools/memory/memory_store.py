...
class LongTermMemoryStore:
    ...
    async def _load_memories(self, api_key: str) -> MemoryCollection:
        ...
        try:
            ...
            print(f"Memory file not found, creating new collection. Details: {e}")
    ...
    async def _save_memories(self, api_key: str, memories: MemoryCollection):
        ...
        files_bytes = json_content.encode('utf-8')
        # 5. Put to cache (kind reminder the key is memory file path)
        self.cache[file_path] = memories
        print(f"Memories saved. Total memories count: {len(memories.memories)}. Updated at: {memories.updated_at.isoformat()}. File path: {file_path}")
    ...
    async def add_memory(self, api_key: str, content: str, importance: float, category: str, topics: list[str]) -> str:
        ...
        return f"Memory added successfully. Total memories count: {len(collection.memories)}."
